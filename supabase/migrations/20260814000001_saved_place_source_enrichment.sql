-- Saved-place source enrichment: never overwrite an attached post.
--
-- No schema change. `saved_places` already carries the single-source model
-- (source_url / source_type, plus the separate generated ai_note); the only
-- defect here is in the UPDATE path of auto_save_share_job_place_result, which
-- rewrote source_type/source_url unconditionally whenever the user already had
-- the place saved.
--
-- Unconditional rewrite is wrong in both directions:
--   * it would silently replace an earlier post (Instagram reel A) with a
--     later, different one (TikTok B) — destroying history and making real
--     multi-source support harder later, and
--   * combined with retries it made the stored source depend on which job ran
--     last rather than on which post the user actually attached first.
--
-- The rule is fill-if-empty, matching lib/savedPlaceSourceMerge.ts (client) and
-- process-share-link/save.ts (metadata path). The saved_places row is already
-- locked FOR UPDATE above, so the guard is race-safe and the whole function
-- stays idempotent under replay.
--
