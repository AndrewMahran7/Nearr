...
assert.equal(state.stage, 'location_background_education', 'foreground access gets its own background-value screen');
state = recordOnboardingBackgroundLocationResult(state, 'granted', at(6)).state;
assert.equal(state.stage, 'notification_education', 'notification education separates the two native permission families');
state = recordOnboardingNotificationResult(state, 'provisional', at(7)).state;
assert.equal(state.stage, 'growing_map');
state = step(state, continueOnboardingToAccount, 8);
...
assert.equal(denied.stage, 'growing_map', 'notification refusal does not block');
...
assert.match(ui, /EXAMPLE REMINDER/);
assert.match(ui, /Future examples—not saved/);
